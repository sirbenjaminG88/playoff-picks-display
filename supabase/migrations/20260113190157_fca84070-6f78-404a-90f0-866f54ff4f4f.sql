-- Fix: Show each user once with their best league's total, exclude Regular Season Beta League
...
BEGIN
  ...
  WITH league_user_points AS (
    -- First, calculate points per user per league
    SELECT 
      up.auth_user_id,
      up.league_id,
    ...
    INNER JOIN leagues l ON l.id = up.league_id
    WHERE up.season = p_season
      AND up.week >= 1 
      AND up.week <= p_through_week
      AND up.week <= 4
      AND l.name != '2025 Regular Season Beta Test'  -- Exclude beta league
      AND l.season_type = 'POST'  -- Only playoff leagues
    GROUP BY up.auth_user_id, up.league_id
  ),
  best_league_per_user AS (
    -- Then, take the best (max) league score for each user
    SELECT 
      lup.auth_user_id,
