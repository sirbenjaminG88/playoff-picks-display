-- Create a function to get global playoff standings across all leagues
-- This returns aggregated user standings without exposing individual picks
...
BEGIN
  ...
  WITH pick_points AS (
    SELECT 
      up.auth_user_id,
      SUM(COALESCE(pws.fantasy_points_standard, 0)) as points
    FROM user_picks up
    ...
    WHERE up.season = p_season
      ...
      AND up.week <= 4  -- Only playoff weeks
